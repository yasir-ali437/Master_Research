...
class FusionEncoder(nn.Module):
    ...
    def forward(self, x_3ch, edge):
        # x0 = torch.cat([x_3ch, edge], dim=1)
...
class Fusion_net(nn.Module):
    def __init__(self, base_ch=32, base_ch2=64):
        ...
        self.encoder = FusionEncoder(input_nc=3, base_ch=base_ch, base_ch2=base_ch2)
        self.decoder = FusionDecoder(base_ch=base_ch, base_ch2=base_ch2)

    def forward(self, mri, pet, mask):
        # edge = self.edge_extractor(mri)
        x_3ch = torch.cat([mri, pet, mask], dim=1)
        feats = self.encoder(x_3ch, edge=None)
