...
class EdgeGuidedFusionLoss(nn.Module):
    ...
    + λaux * |Ehat - ∇MRI|
    + λtv  * TV(Y)
    + λmask_p * (masked PET-priority term)
    """
    def __init__(self,
                 ...
                 w_pet_sal=1.0,
                 w_mask_pet=0.5):   # NEW: PET-priority weight on mask
        super().__init__()
    ...
    def forward(self, y, mri, pet, ehat, mask):
        # ---------- MS-SSIM terms ----------
        ssim_m = 1.0 - ssim_fn(y, mri, data_range=1.0)
        ssim_p = 1.0 - ssim_fn(y, pet, data_range=1.0)

        # ---------- Global L1 to both sources ----------
        l1_m = self.l1(y, mri)
        l1_p = self.l1(y, pet)

        # ---------- PET saliency (soft mask) ----------
        ...
        # ---------- NEW: PET-priority term on binary mask ----------
        # mask: binary {0,1}, shape [B,1,H,W] or [B,H,W]
        mask = mask.to(y.dtype)
        if mask.dim() == 3:        # [B,H,W] -> [B,1,H,W]
            mask = mask.unsqueeze(1)

        # normalised masked L1 to PET (only where mask==1)
        # This gives extra “pull” of Y toward PET in masked regions.
        masked_pet_l1 = (mask * (y - pet).abs()).sum() / (mask.sum() + 1e-6)

        # ---------- Gradient consistency to MRI ----------
        ...
        loss = (
            ...
            self.w_mask_pet * masked_pet_l1   # NEW TERM
        )
        ...
        terms = {
            ...
            "l1_pet_sal": l1_pet_sal.item(),
            "masked_pet_l1": masked_pet_l1.item()
        }
        return loss, terms
...
def main():
    ...
    parser.add_argument("--data_root", type=str, default="/mnt/yasir/PET_Dataset_Processed/Separated_Centered_PET_MRI_Updated_Dataset_Splitted/train", help="contains mri/ and pet/")
