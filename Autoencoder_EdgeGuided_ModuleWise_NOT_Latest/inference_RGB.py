...
def load_model(ckpt_path: str, device: str = "cuda"):
    model = Fusion_net(input_nc=4, output_nc=3).to(device)
...
def main():
    ...
    for s in common:
        ...
        pet_img_y = to_tensor(pet_img_rgb).float()
        
        # pet_img_y = pet_img_y/255.0
        # mri = mri/255.0
        
        fused, edge = fuse_pair(model, mri, pet_img_y, mask, device=args.device, amp=args.amp)
        out_path = os.path.join(args.out_dir, f"{s}_fused.png")
        
        # # save_gray(fused, out_path)
        fuseImage = fused;   #400 for edge guided, 500 for edge dominant
        fuseImage = fuseImage.permute(1, 2, 0).cpu().numpy()
        # If the image is in float32 (range [0, 1]), scale it to [0, 255] and convert to uint8
        if fuseImage.max() <= 1.0:  # Assuming the image is normalized in the range [0, 1]
            fuseImage = (fuseImage * 255).astype(np.uint8)
        else:  # If it's already in [0, 255], just convert to uint8 if it's not already
            fuseImage = fuseImage.astype(np.uint8)
        
        # # file_name = 'fuse'+str(index) + '.png'
        output_path = out_path 
        print("Saving to ",fuseImage.shape)
        # save_gray(fused, out_path)
