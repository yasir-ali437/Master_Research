...
class EdgeGuidedFusionLoss(nn.Module):
    ...
    def multiscale_transport(self, y, pet, mask, mri, scales=[0.25], max_points=4096):
        """
        Multi-scale, memory-efficient Sinkhorn transport between PET and fused images.
        Works per-batch sample with masking and subsampling.

        Args:
            y:      Fused image tensor [B, C, H, W]
            pet:    PET image tensor [B, C, H, W]
            mask:   Binary/soft cortex mask [B, 1, H, W]
            scales: List of scale factors for multi-scale OT
            max_points: Max number of sampled points per image for Sinkhorn

        Returns:
            Average transport loss over batch and scales
        """
        B = pet.size(0)
        ...
        for scale in scales:
            # print(f"Processing scale: {type(scale)}, {scale}")
            # Rescale the inputs
            # y_scaled = F.interpolate(y, scale_factor=scale, mode='bilinear', align_corners=False)
            # pet_scaled = F.interpolate(pet, scale_factor=scale, mode='bilinear', align_corners=False)
            # mri_scaled = F.interpolate(mri, scale_factor=scale, mode='bilinear', align_corners=False)
            # mask_scaled = F.interpolate(mask, scale_factor=scale, mode='nearest')  # Use nearest for mask
            ...
            # Calculate transport and SSIM losses for the current scale
            for i in range(B):
                pet_i = (pet_scaled[i]).view(3, -1).T
                y_i = (y_scaled[i] * mask_scaled[i]).view(3, -1).T
                valid = mask_scaled[i].flatten() > 0.1
                ...
                L_out += torch.mean((y_scaled[i] * (1 - mask_scaled[i])) ** 2)
                
                # Ensure same channels before SSIM
                mri_i_rgb = mri_scaled[i:i+1].repeat(1, 3, 1, 1)
                L_ssim += 1 - ssim(y_scaled[i:i+1]*mask_scaled[i:i+1], mri_i_rgb, data_range=1.0)
            
            # Average the losses across batch for the current scale
            L_transport /= B
            L_out /= B
            L_ssim /= B

            # Optionally, you can apply a weight for each scale (e.g., larger scale might matter more)
            scale_weight = 1.0 if scale == 1 else 0.5 if scale == 0.5 else 0.25

            # Aggregate the loss terms with scale weights
            loss_transport = self.alpha * L_transport * scale_weight
            loss_out = self.beta * L_out * scale_weight
            loss_ssim = self.gamma * L_ssim * scale_weight

            # Add to total loss
            L_transport += loss_transport
            L_out += loss_out
            L_ssim += loss_ssim
        
        total_loss = L_transport + L_out + L_ssim
        return total_loss

    def forward(self, y, mri, pet, mask, ehat, epoch):
        mri_i_rgb = mri.repeat(1, 3, 1, 1)
        ssim_m = 1.0 - ssim_fn(y, mri_i_rgb, data_range=1.0)
        ssim_p = 1.0 - ssim_fn(y, pet, data_range=1.0)
        l1_m, l1_p = self.l1(y.mean(1,keepdim=True), mri_i_rgb), self.l1(y, pet)
        ...
        # new_blur = max(self.min_blur, self.base_blur * (self.decay ** epoch))
        # self.current_blur = new_blur
        # self.sinkhorn = SamplesLoss("sinkhorn", p=2, blur=new_blur)
        
        # Instead of 3D color channels only, concatenate position info
        # H, W = pet.shape[-2:]
        # coords = torch.stack(torch.meshgrid(
        #     torch.linspace(-1, 1, H, device=pet.device),
        #     torch.linspace(-1, 1, W, device=pet.device)), dim=-1)

        # B = pet.size(0)
        # L_transport, L_out, L_ssim = 0.0, 0.0, 0.0
        
        # for i in range(B):
        #     feat_pet = torch.cat([(pet[i]*mask[i]).permute(1,2,0), coords], dim=-1).view(-1, 5)
        #     feat_y   = torch.cat([(y[i]*mask[i]).permute(1,2,0), coords], dim=-1).view(-1, 5)
        #     valid = mask[i].flatten() > 0.1
        #     feat_pet, feat_y = feat_pet[valid], feat_y[valid]
        #     L_transport += self.sinkhorn(feat_y, feat_pet)
        ...
        # pet = F.interpolate(pet, scale_factor=0.25, mode='bilinear', align_corners=False)
        # mri = F.interpolate(mri, scale_factor=0.25, mode='bilinear', align_corners=False)
        # mask = F.interpolate(mask, scale_factor=0.25, mode='nearest')  # Use nearest for mask
        ...
        # loss_transport = self.multiscale_transport(y, pet, mask, mri)
        ...
        total_loss =  (
            ...
            2*loss_transport + 2.5*L_intensity + 0.2 * L_smooth + L_edge_penalty + mass_loss*0.00005 + 0.5*ssim_m + 0.5*l1_m )
